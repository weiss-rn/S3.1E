...
MONGO_HOST = os.environ.get('MONGO_HOST', '100.103.170.96')
...
def index():
    ...
    if search_query:
        ...
        try:
            ...
            pass
        try:
            or_clauses.append({'stok': int(search_query)})
        except ValueError:
            pass
...
def add():
    if request.method == 'POST':
        ...
        try:
            ...
            flash("Harga must be a number.", "error")
            return render_template('add.html')
        ...
        if stok_raw:
            try:
                ...
                flash("Stok must be an integer.", "error")
                return render_template('add.html')
...
def edit(id):
    ...
    if request.method == 'POST':
        ...
        collection.update_one(
            {'_id': obj_id},
            {'$set': {'kode': kode, 'nama': nama, 'harga': harga, 'image': image_filename}}
        )
